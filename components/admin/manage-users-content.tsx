...
export function ManageUsersContent({
  ...
  const handleSearch = async () => {
    ...
    if (trimmed.length < 4) {
      setSearchError('Digite um código válido (mínimo 4 caracteres)');
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/users/associate?code=${encodeURIComponent(trimmed)}`
      );
      const data = await res.json();

      if (!res.ok) {
        setSearchError(data.error || 'Usuário não encontrado');
        return;
      }
  ...
  const handleAssociate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || 'Erro ao associar usuário');
        return;
      }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Associe novos usuários e gerencie o acesso da sua equipe
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            Associar Novo Usuário
          </CardTitle>
          <CardDescription>
            Digite o código de 6 caracteres que o usuário recebeu ao criar a
            conta
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={isSearching || isAssociating || code.trim().length < 4}
              className="gap-2"
            >
              {isSearching ? (
              ...
              {tenantUsers.map((u) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {u.modules.length > 0 ? (
                      u.modules.map((mod) => {
                        const moduleInfo = featureModules.find(
                          (fm) => fm.key === mod
                        );
                        return (
                          <Badge
